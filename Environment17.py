...
class Environ:
    ...
    ######################       更新车辆信道          ###########################################
        ...
        def renew_channels_fastfading(self, h, velocity_A, velocity_B):
                ...
                # 保存当前的信道状态供下一步使用
                self.V2V_channels_fastfading_prev = self.V2V_channels_fastfading

                # 将快速衰落效应叠加到信道模型上
                self.V2V_channels_with_fastfading = V2V_channels_with_fastfading - 20 * np.log10(
                    np.abs(self.V2V_channels_fastfading)
                )
                ...
                # 快速衰落（类似 V2V 的逻辑）
                fading_real = np.random.normal(0, 1, V2I_channels_with_fastfading.shape)
                fading_imag = np.random.normal(0, 1, V2I_channels_with_fastfading.shape)

                if hasattr(self, 'V2I_channels_fastfading_prev'):
                    self.V2I_channels_fastfading = (
                            correlation_coefficient * self.V2I_channels_fastfading_prev
                            + math.sqrt(1 - correlation_coefficient ** 2)
                            * (fading_real + 1j * fading_imag) / math.sqrt(2)
                    )
                else:
                    self.V2I_channels_fastfading = (fading_real + 1j * fading_imag) / math.sqrt(2)

                self.V2I_channels_fastfading_prev = self.V2I_channels_fastfading



                # 更新 V2I 快速衰落信道
                self.V2I_channels_with_fastfading = V2I_channels_with_fastfading - 20 * np.log10(
                    np.abs(self.V2I_channels_fastfading)
                )
        ...
        def Compute_Performance_Reward_Train(self, actions_power):
            ...
            for rb in range(nRB):
                ...
                # 该 RB 上各条 V2V 的自信号 + 互相干扰
                for u in range(L):
                    i, j = idxs[u]
                    rx_ij = self.vehicles[i].destinations[j]  # V2V 的接收端 ID（邻居）


                    V2V_Signals[i, j] = Psel[i, j] * 10 ** (
                            (30 - self.V2V_channels_with_fastfading[i, rx_ij, rb]
                             + 2 * self.vehAntGain - self.vehNoiseFigure) / 10.0)
                    ...
                    for v in range(L):
                        if v == u:
                            continue
                        k, l = idxs[v]
                        rx_kl = self.vehicles[k].destinations[l]

                        V2V_Interference[i, j] += Psel[k, l] * 10 ** (
                                (30 - self.V2V_channels_with_fastfading[k, rx_ij, rb]
                                 + 2 * self.vehAntGain - self.vehNoiseFigure) / 10.0)


                    V2I_Interference[rb] += Psel[i, j] * 10 ** (
                            (30 - self.SV2I_channels_with_fastfading[i]
                             + self.vehAntGain + self.bsAntGain - self.bsNoiseFigure) / 10.0)


            if consider_v2i_to_v2v_interf:
                for rb in range(nRB):
                    idxs = np.argwhere(actions == rb)
                    if len(idxs) == 0:
                        continue
                    ...
                    for (i, j) in idxs:
                        rx_ij = self.vehicles[i].destinations[j]
        ...
        def act_for_training(self, actions):
            ...
            lambdda1 = 0.5
            lambdda2 = 1 - lambdda1
            reward = lambdda1 * np.mean(HumanDriven_rewards) + lambdda2 * np.mean(Autonomous_rewards)
